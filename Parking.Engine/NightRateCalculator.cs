...
{
	public class NightRateCalculator : ICalculator<NightRateCalculator>
	{
		private readonly IModelBuilder<string, IEnumerable<RefData>> _refDataBuilder;
		private const string NightRate = "Night Rate";
		public NightRateCalculator(IModelBuilder<string, IEnumerable<RefData>> refDataBuilder)
		{
		...
		{
			var refDatas = _refDataBuilder.Build(NightRate).ToList();
		...
		{
			return new Rate { TimeSlot = timeSlot, ParkingRate = new ParkingRate { Amount = amount, Name = NightRate } };
		}
	}
}